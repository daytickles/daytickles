...
// cloud-side a share row could even reference.
//
// cardImageUri is required, not optional — unlike shareEntry there's no
// text-message fallback available here (no entry text exists to fall
// back to), so the caller is expected to have already generated it (or
// to not call this at all if generation failed).
export async function sharePhoto({ profile, captionId, onProfileUpdated, cardImageUri }) {
  const now = new Date();
  const caption = SHARE_CAPTIONS.find((c) => c.id === captionId);
