...
import { toLocalDateString, parseLocalDateString } from './week';
...
// full 30-day period that's elapsed since it was last touched. Local-time
// throughout (see lib/week.js) -- share_period_start is a plain local
// calendar date, not a UTC instant, same convention as entry_date.
function currentPeriod(profile, now) {
  let periodStart = parseLocalDateString(profile.share_period_start);
...
async function checkAndConsumeShareCap(profile, now, onProfileUpdated) {
  ...
  if (!status.unlimited) {
    ...
    const { error } = await supabase
      .from('profiles')
      .update({
        share_period_start: toLocalDateString(periodStart),
...
export async function shareEntry({ profile, entry, captionId, onProfileUpdated, cardImageUri, logCaptionShare }) {
  const now = new Date();
...
// above): { missingPhoto: true } | { blocked: true, cap } |
// { captureFailed: true } | the underlying shareEntry result on success.
export async function sharePhotoOnlyEntry({ profile, entry, photoUri, captureCard, accentColor, onProfileUpdated }) {
  if (!photoUri) return { blocked: false, missingPhoto: true };

  const status = shareStatus(profile);
