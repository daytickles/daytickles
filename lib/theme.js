...
// VIBE_COLORS' own values, for use as a small ICON's color specifically
// -- as opposed to a bold fill/background (VibeCard's tiles,
// rhythmLegendDot, the Weekly Vibes bar chart), which stay the raw
// vivid value on purpose. At icon size, on a white/cream/near-white
// backing, received and self measured well under WCAG's 3:1 minimum for
// graphical objects (1.54:1 / 2.01:1 vs white) -- both thin "-outline"
// glyphs like given's, so this is a contrast problem, not a weight one.
// darken(..., 0.4) clears 3:1 for both against white AND the cream
// background some of these icons render on (self 4.12/3.75, received
// 5.14/4.67) -- weekly-summary.js's own separate 0.3 precedent
// (rhythmTotalPill) doesn't quite reach 3:1 against cream, so this uses
// a stronger amount rather than reusing that value as-is. given/purple
// is already 7.7:1+ and returned unchanged.
export function vibeIconColor(nature) {
  if (nature === 'given') return VIBE_COLORS[nature];
  return darken(VIBE_COLORS[nature], 0.4);
}
...
// a photo-only Tickle's Polaroid caption (EntryCard.js). day_journal is
// included here even though it isn't a Vibe (see VIBE_COLORS/NATURE_ORDER
// above, which deliberately exclude it) -- a photo-only My Day entry
// still hits this same Polaroid caption lookup, and with no entry here
// it rendered blank instead of a real label.
...
// Labels changed 2026-09-04 from the "-weaver" proper nouns (Wordweaver/
// Soulweaver/Wittweaver) to plain descriptive phrases -- display text
// only, the wordweaver/soulweaver/wittweaver keys below and their colors
// are unchanged (award_type is a real, constrained DB column across two
// tables, see migration 0020 -- not just a JS key). The single `label`
// field replaces the old two-field label+description pair (description
// is gone; nothing else read it).
export const AWARD_TYPES = {
  wordweaver: {
    label: "Beautifully expressed",
    color: "#3E7CE0",
  },
  soulweaver: {
    label: "Touching or heartfelt",
    color: "#7C2B82",
  },
  wittweaver: {
    label: "Witty, amusing or entertaining",
    color: "#E25068",
  },
  // Added 2026-09-06. Green, H=116 S=55% L=45% -- hue-verified the same
  // way as the original three (computed HSL against every chromatic
  // color in this file): sits in the single largest open gap on the
  // whole wheel (91 deg sage theme -> 142 deg FOUNDING_MEMBER_BADGE_COLOR),
  // 25-26 deg clear on both sides -- comfortably past every existing
  // award precedent (wittweaver 15.7 deg, soulweaver 9 deg). No other
  // color in this file is green. A "laughing-yellow" gold was considered
  // and rejected -- the 0-65 deg band is this wheel's most crowded
  // stretch (same problem that forced wittweaver off its originally-
  // requested coral-orange), best open sub-gap there is only ~15 deg.
  lol: {
    label: "LOL",
    color: "#3CB234",
  },
};
export const AWARD_ORDER = ["lol", "wordweaver", "soulweaver", "wittweaver"];

// Photo-only Tickles swap ONE of the three choices contextually --
// "Beautifully expressed" is text-specific (about eloquent writing), so
// a photo-only entry gets a different, visually-scoped phrase for the
// exact same wordweaver identifier/color instead (no new color -- this
// isn't a genuine new type, just contextual display text on the
// existing one). The other two work equally well for a photo, so
// they're untouched. Never a genuine 4th choice -- every entry always
// shows exactly 3, this is pure display-text branching.
export const AWARD_LABEL_PHOTO_ONLY_OVERRIDE = {
  wordweaver: "Visually on point, striking, creative",
};

// Single source for "what text does this award type show" -- used by
// both the give-picker (AwardPickerModal) and the public badge's
// tap-to-reveal (EntryCard), so the two can never drift on which phrase
// applies to a photo-only entry's wordweaver slot. isPhotoOnly is the
// viewer's own already-known entry.entry_kind === 'photo_only' check;
// this function doesn't look it up itself.
export function awardLabelFor(type, isPhotoOnly) {
  if (isPhotoOnly && AWARD_LABEL_PHOTO_ONLY_OVERRIDE[type]) return AWARD_LABEL_PHOTO_ONLY_OVERRIDE[type];
  return AWARD_TYPES[type]?.label;
}
