...
// reading star/thumbs-up/cafe left to right.
...
// The 4 fixed grayscale fills in assets/backgrounds/WALLPAPER01.svg,
// darkest -> lightest. Retints the illustration to a person's chosen
// accent color while preserving each layer's relative lightness order:
// each gray's position on the black->white ramp (0-1) is fed straight
// into darken()/lighten() above rather than any new color math, so a
// value below the midpoint darkens the accent and one above it lightens
// it, by an amount proportional to how far from the midpoint it sits.
const WALLPAPER_GRAYS = [91, 136, 195, 254];
export function wallpaperTints(accentHex) {
  return WALLPAPER_GRAYS.map((gray) => {
    const fraction = gray / 255;
    return fraction < 0.5
      ? darken(accentHex, (0.5 - fraction) * 2)
      : lighten(accentHex, (fraction - 0.5) * 2);
  });
}
