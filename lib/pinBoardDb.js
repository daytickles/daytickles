...
// Relink -- swaps a photo-only entry's current (missing/broken-file)
// photo for a freshly-picked one. The freshly-picked file becomes a
// genuine new pinned_photos row (so it also shows up in Tickle Pics
// itself, same as any other pinned photo), linked to this entry;
// oldPhotoId's own link is dropped so the entry ends up pointing at
// exactly one photo again. oldPhotoId's now-orphaned row/file is left
// alone, same pre-existing gap as the TODO on deletePinnedPhoto above --
// not a new problem introduced here.
export async function relinkPhotoToEntry(userId, oldPhotoId, entryId, newFilePath) {
  const newPhotoId = await addPinnedPhoto(userId, newFilePath);
  if (oldPhotoId != null) await unlinkPhotoFromEntry(userId, oldPhotoId, entryId);
  await linkPhotoToEntry(userId, newPhotoId, entryId);
  return newPhotoId;
}
...
// All entry_ids a photo is linked to -- used by the delete flow to tell
// apart the two deletion scenarios (a photo-only Tickle's own entry vs.
// a separately-written entry it's merely pinned to; a photo can be
// linked to both at once). Returns cloud tickle_entries ids only; the
// caller still needs a Supabase lookup to get each one's entry_kind.
export async function getEntryIdsForPhoto(userId, photoId) {
  const database = getDb(userId);
  const rows = await database.getAllAsync(
    'SELECT entry_id FROM photo_entry_links WHERE photo_id = ?',
    photoId
  );
...
// Batched version of getPhotoForEntry for a known set of entry ids --
// used by Feed/Calendar to resolve every currently-loaded photo-only
// entry's local photo in one query, instead of one query per card. Same
// "first link wins" tie-break as getPhotoForEntry's LIMIT 1, applied in
// JS here since SQLite has no per-group LIMIT.
export async function getPhotosForEntries(userId, entryIds) {
  if (!entryIds.length) return new Map();
  const database = getDb(userId);
  const placeholders = entryIds.map(() => '?').join(',');
  const rows = await database.getAllAsync(
    `SELECT photo_entry_links.entry_id AS entry_id, pinned_photos.*
     FROM photo_entry_links
     JOIN pinned_photos ON pinned_photos.id = photo_entry_links.photo_id
     WHERE photo_entry_links.entry_id IN (${placeholders})`,
    ...entryIds
  );
  const map = new Map();
...
// Every pinned photo plus its own links, for lib/photoBackup.js's export
// scan -- two queries total regardless of board size, rather than
// getEntryIdsForPhoto called once per photo (an N+1 that'd otherwise
// scale with however many photos are pinned).
export async function getAllPinnedPhotosWithLinks(userId) {
  const database = getDb(userId);
  const photos = await database.getAllAsync('SELECT * FROM pinned_photos ORDER BY position ASC');
  const links = await database.getAllAsync(
    'SELECT photo_id, entry_id, linked_at FROM photo_entry_links'
  );

  const linksByPhoto = new Map();
  for (const link of links) {
    if (!linksByPhoto.has(link.photo_id)) linksByPhoto.set(link.photo_id, []);
    linksByPhoto.get(link.photo_id).push({ entryId: link.entry_id, linkedAt: link.linked_at });
  }

  return photos.map((photo) => ({ ...photo, links: linksByPhoto.get(photo.id) || [] }));
}
