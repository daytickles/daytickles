...
//
// Public-upload mechanic, originally built for photo-only Tickles --
// Part 4 of that feature (see supabase/migrations/0056) -- and reused
// as-is for a text entry's Tickle-a-Photo link (a Ripple-time upload
// mechanic, not a new feature): neither makePhotoTicklePublic nor
// makePhotoTicklePrivate below ever reference entry_kind, since both
// already worked purely off entry.user_id/entry.id/entry.media_url.
// Upload only ever happens at the moment an existing PRIVATE entry is
// toggled public, never at creation time -- a private one stays
// strictly local-only (device gallery + pinBoardDb.js's SQLite DB),
// matching how photo-only already treated "private" (migration 0055's
// own comment on local_photo_filename).
...
// Best-effort cleanup for an entry's uploaded Storage object, called
// alongside (not blocking) the entry's own tickle_entries delete -- see
// each screen's own handleDeleteEntry. Deliberately never throws -- a
// Storage cleanup failure is real but unrelated to the user's own
...
// makePhotoTicklePublic above) -- this is the only gate now, not
// entry_kind, since a text entry with a Tickle-a-Photo link can carry
// media_url too (see makePhotoTicklePublic/Private's own callers).
//
