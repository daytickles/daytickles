...
export async function regenerateAwarenessCueSchedule({
  ...
  for (let dayOffset = 0; dayOffset < AWARENESS_CUE_BATCH_DAYS; dayOffset++) {
    ...
    const effectiveStart = dayOffset === 0 && windowStart < now ? now : windowStart;
    ...
    const dayCount = Math.min(targetCount, AWARENESS_CUE_MAX_DAILY_COUNT);

    const spanMs = windowEnd.getTime() - effectiveStart.getTime();
    for (let i = 0; i < dayCount; i++) {
      times.push(new Date(effectiveStart.getTime() + Math.random() * spanMs));
    }
...
// `timestamp` (ms epoch, see NotificationScheduler.kt's DateTrigger),
// while iOS implements a one-off DATE trigger as a non-repeating
// UNCalendarNotificationTrigger under the hood, so it reads back as a
// `calendar` trigger with full dateComponents instead (see
// SchedulerModule.swift) -- no `timestamp` field at all there. No iOS
