...
// no separate toggle. Returns the single local 'YYYY-MM-DD' date that is
// currently eligible for a Day Dots prompt: today, once today's evening
// reminder time has actually passed, otherwise yesterday (covering
// someone who opens the app before tonight's reminder but never
// answered last night's). Only this one date is ever checked by the
// caller (see app/(tabs)/home.js) -- there is deliberately no scan back
// through every previously-missed date. Once a newer date becomes
// eligible, older unanswered ones are simply never queried again, which
// is the entire mechanism behind "quietly fade away" (see migration
// 0060's own header comment) -- no explicit cleanup step exists or is
// needed.
export function currentDayDotsPromptDate() {
  const now = new Date();
  const eveningFired =
    now.getHours() > EVENING_HOUR ||
    (now.getHours() === EVENING_HOUR && now.getMinutes() >= EVENING_MINUTE);
  return localDateString(eveningFired ? 0 : 1);
}
