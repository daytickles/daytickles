...
// The reverse of toLocalDateString -- parses a stored 'YYYY-MM-DD' date
// column back into a real local-midnight Date, for callers that need to
// do arithmetic on it (e.g. lib/sharing.js's 30-day share-cap rollover).
// `new Date(dateStr)` alone would parse it as UTC midnight instead --
...
// All 7 'YYYY-MM-DD' dates of the current week (local), in weekStartDay
// order -- e.g. Weekly Summary's rhythm chart, one bar per day, needs
// the actual day order a person chose in Settings rather than an
// assumed Mon-Sun layout (same weekStartDay convention as everything
// else in this file). Days later in an in-progress week are still
// included even though they haven't happened yet -- callers simply get
// zero counts for those, same as any other "this week so far" query.
export function currentWeekDates(weekStartDay = DEFAULT_WEEK_START_DAY) {
  const start = weekStartOf(new Date(), weekStartDay);
