...
// photos + private Photo-Only Tickle photos and Tickle-a-Photo links
// never made public) into a single portable container file -- the only
// way any of this data leaves the device, since none of it is ever
// synced to Supabase (see lib/pinBoardDb.js's own header comment). Any
// entry (photo-only or a text entry with a Tickle-a-Photo link) whose
// linked photo is already public is deliberately excluded: its bytes
// already live in Supabase Storage (photo-tickle-media bucket) once
// media_url is set, and every render path that resolves a linked
// photo's image (feed.js/calendar.js's own resolveLinkedPhotoUris)
// already falls back to entry.media_url whenever no local copy exists,
// so omitting it here costs nothing -- confirmed by reading that
// fallback, not assumed.
//
...
export async function planPhotoBackup(userId) {
  ...
  // Not scoped to entry_kind='photo_only' -- a text entry's
  // Tickle-a-Photo link now goes through the same upload-on-make-public
  // mechanic (lib/photoTickleStorage.js), so its bytes are just as
  // safely in Storage as a photo-only entry's once media_url is set.
  let alreadyUploadedEntryIds = new Set();
  if (allEntryIds.length) {
    ...
    alreadyUploadedEntryIds = new Set((data || []).filter((e) => e.media_url).map((e) => e.id));
  }

  // Skip a photo only if EVERY one of its links points at an entry
  // whose media_url is already set -- any other link (a written-Tickle
  // link or a photo-only entry that's still private, or never went
  // through the upload mechanic, or no link at all) means this device
  // is the only copy that exists, so it stays in.
  const included = photos.filter(
    (photo) => photo.links.length === 0 || !photo.links.every((l) => alreadyUploadedEntryIds.has(l.entryId))
  );
