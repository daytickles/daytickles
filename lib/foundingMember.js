...
export const MONTHLY_REQUIREMENTS = [
  { key: 'shared_to_feed_count', target: 6, label: 'Shared Tickles to Tickle Stash' },
  ...
  { key: 'made_me_smile_count', target: 8, label: 'Tickles tagged "Made me smile"' },
  { key: 'paying_forward_count', target: 2, label: 'Tickles tagged "Paying forward"' },
  { key: 'mood_boost_count', target: 4, label: 'Tickles tagged "For me"' },
];

// Also hardcoded as `interval '14 days'` in opt_in_to_founding_member
// and ensure_founding_member_enrollment (supabase/migrations/0049) --
// same two-sources-of-truth situation as MONTHLY_REQUIREMENTS above,
// for the same reason (the server's copy is the authoritative one that
// actually locks in expiry; this one is just for computing display
// windows/countdowns -- exported so app/founding-member.js's own
// days-remaining display uses this same number rather than a third
// copy of it).
export const OPT_IN_COOLDOWN_DAYS = 14;
...
export async function fetchEnrollment(userId) {
...
// Read-only inputs for Home's pace-reminder banner (Stage 8) -- unlike
// advanceFoundingMemberProgress, this never writes anything (no month
// evaluation, no referral-slot/code side effects), so it's cheap enough
// to call on every Home focus rather than being confined to the FM
// page. Returns null when there's no active enrollment to nudge about;
// otherwise the current checkpoint window plus whichever of the 7
// monthly requirements are currently below half their target -- Home
// itself decides the midpoint/window-end timing gate and the dismissal
// check, both of which only need window.startISO/endISOExclusive.
export async function fetchFoundingMemberPaceStatus(userId) {
  const enrollment = await fetchEnrollment(userId);
  if (!enrollment || enrollment.status !== 'active') return null;

  const attempt = enrollment.restart_count + 1;
  const evaluated = await fetchEvaluatedMonthIndexes(userId, attempt);
  const monthIndex = Math.min(evaluated.length ? Math.max(...evaluated) + 1 : 1, 6);
  const window = checkpointWindow(enrollment.attempt_started_at, monthIndex);
  const progress = await fetchMonthProgress(userId, window);

  const laggingRequirements = MONTHLY_REQUIREMENTS.filter(
    (r) => (progress?.[r.key] || 0) < r.target / 2
  );

  return { window, laggingRequirements };
}

// Read-only input for Home's opt-in cooldown reminder -- sibling to
// fetchFoundingMemberPaceStatus above, same reasoning for why it's safe
// to call on every Home focus (no writes). Returns null once there's
// nothing to nudge about (not pending_opt_in at all -- already opted
// in, or already terminal). The window's start is the enrollment row's
// own created_at (see migration 0049 -- there's no separate cooldown-
// deadline column, it's always derived from this), so Home's existing
// midpoint-through-end/dismissal math (fetchFoundingMemberPaceStatus's
// own caller already implements this) applies unchanged to this window
// too.
export async function fetchFoundingMemberOptInReminderStatus(userId) {
  const enrollment = await fetchEnrollment(userId);
  if (!enrollment || enrollment.status !== 'pending_opt_in') return null;

  const startISO = enrollment.created_at;
  const endISOExclusive = new Date(
    new Date(enrollment.created_at).getTime() + OPT_IN_COOLDOWN_DAYS * 24 * 60 * 60 * 1000
  ).toISOString();

  return { window: { startISO, endISOExclusive } };
}
...
// The one distinct, explicit opt-in action (a real button in
// app/founding-member.js's pending_opt_in branch) that starts the real
// 6-month clock -- merely opening/visiting the FM page never calls
// this. Idempotent against a repeat call once already active/terminal
// (see 0049); if the 14-day cooldown already lapsed by the time this
// lands, the server routes it straight to the same closed terminal
// state ensure_founding_member_enrollment's own lazy expiry check would
// otherwise take.
export async function optInToFoundingMember(userId) {
  const { data, error } = await supabase.rpc('opt_in_to_founding_member', {
    p_user_id: userId,
  });
  if (error) throw error;
  return data;
...
// Ends the Founding Member enrollment permanently -- the "Take part"
// toggle's only path to turning itself off, per the post-launch
// addendum. Drives the same terminal state as genuinely failing the
// quest (see supabase/migrations/0031's fail_founding_member_
// enrollment helper); idempotent against an already-terminal
// enrollment (returns { opted_out: false } rather than erroring).
export async function optOutOfFoundingMember(userId) {
  const { data, error } = await supabase.rpc('opt_out_of_founding_member', {
    p_user_id: userId,
  });
  if (error) throw error;
  return data;
